...
async function main() {
  ...
  // Create a paymasterAPI object for the paymaster.
  const paymasterAPI = new VerifyingPaymasterAPI(
    config.verifyingPaymasterUrl,
    config.entryPoint
  );
  ...
  const paymasterAndData = await paymasterAPI.getPaymasterAndData(op);
  console.log("paymasterAndData", paymasterAndData);

  // Add the signed paymaster data to the user operation that has been signed by the account
  const signedOp = await accountAPI.signUserOp({
    ...op,
    paymasterAndData,
  });
  ...
  const uoHash = await client.sendUserOpToBundler(signedOp);
